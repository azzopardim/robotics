...
int fadeStep = 0;
int fade_steps = 400;
unsigned int pattern = 0;                  //set which light pattern is used
unsigned int pattern_int = 4;            //set pattern delay between
...
void setup() {
  ...
  Serial.println("Arduino Nano Setup");
  ...
  // RESET PIN
  if (debug1) {Serial.println("Setting RESET PIN mode");}
  digitalWrite(RESET_PIN, HIGH);
  delay(20);
  pinMode(RESET_PIN, OUTPUT);  

  // WIRE
  if (debug1) {Serial.println("Setting WIRE for receiving events");}
  Wire.begin(SLAVE_ID); 
  ...
  // LED
  if (debug1) {Serial.println("Setting LED PIN mode");}
  pinMode(LED_BRIGHT_PIN, INPUT);

  //OLED
  if (debug1) {Serial.println("Setting OLED PIN mode");}
  pinMode(OLED_ACTIVE_PIN, INPUT);
  digitalWrite(OLED_ACTIVE_PIN, LOW);

  // RGB
  if (debug1) {Serial.println("Setting RGB brightness and resetting LEDs");}
  if (rgb_active) {
    led_eyes.begin();
  ...
  if (debug1) {Serial.println(F("Arduino Ready!"));}
  
  pattern = 4; //michael: changed from 12 to 4
...
/*int get_distance(int side) {
  unsigned int dist = sonar[side].ping_cm();
